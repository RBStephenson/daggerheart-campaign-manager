"""Tests for the read-only bestiary reference endpoint."""
...
def test_returns_404_when_flag_off(as_user) -> None:
    client = as_user("gm")
    assert client.get("/api/bestiary/").status_code == 404


def test_requires_auth_when_flag_on(client: TestClient, db: Session) -> None:
    _enable(db)
    assert client.get("/api/bestiary/").status_code == 401


def test_requires_gm_role_when_flag_on(as_user, db: Session) -> None:
    _enable(db)
    resp = as_user("player").get("/api/bestiary/")
    assert resp.status_code == 403


def test_returns_dataset_when_enabled(as_user, db: Session) -> None:
    _enable(db)
    resp = as_user("gm").get("/api/bestiary/")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data["adversaries"]) == 129
    assert len(data["environments"]) == 19
